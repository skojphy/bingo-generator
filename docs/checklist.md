...
- [x] 빙고판 각 칸을 정사각형 textarea로 변경 및 스타일 개선 (커밋: 5c313f2)
    - aspect-ratio: 1/1로 셀 비율 완전 고정, 긴 텍스트 입력 시에도 세로 확장 방지
- [x] 빙고판 5x5 grid 레이아웃 및 gap/여백 스타일 개선 (커밋: cad9d4d)
    - vw 단위 width/height, padding, margin: 0 auto 등으로 어떤 해상도에서도 중앙+여백+정사각형 유지
- [x] 5x5 빙고판 생성 로직 구현
- [x] 칸별 스타일 적용
...
- [x] 공유/적용 버튼 하단에 별도 배치, 반응형 스타일(여백 축소, 폰트/너비/줄바꿈) 적용
- [x] 공유 URL(/share/{id}) 접근 시 Notion에서 데이터 불러오기 (Board.svelte 내 임시 구현)
- [x] 공유 URL 복사, 입력창 등 모바일/데스크톱 모두에서 사용성 확보
...
- [x] 각 셀에 접근성(role, aria, tabindex 등) 속성 적용
- [ ] 빙고판 이미지 다운로드 (예정)
...
- [x] 불필요/중복 CSS, min-width/margin-left/right 등 제거
